...
```{r}
#Use ggplot
nTREP <- ncol(TREP_all_data)-1

par(mfrow=c(1,3))

ggplot(as.data.frame(TREP_all_data), aes(x=date)) +
  geom_line(aes(y=original.TREP),col='black') +
  geom_line(aes(y=Diff.TREP),col='blue') +
  geom_line(aes(y=Detrend.TREP),col='green') +
  ylab(paste0("Trillion Btu")) + 
  ggtitle('All TREP Graphs') +
  theme(legend.position = "right",plot.title = element_text(size = rel(1.2), lineheight = .9,)) +
  scale_color_manual(name='TREP Legend',
                   breaks=c('Original','Diff','Detrend'),
                   values=c('Original'='black','Diff'='blue','Detrend'='green'))
```
...
par(mfrow=c(1,3))
for(i in 1:(nTREP)){
  Acf(TREP_all_data[,i],lag.max=40,main=paste("ACF",colnames(TREP_all_data)[(1+i)],sep=""),ylim=c(-0.5,1)) 
}
```
