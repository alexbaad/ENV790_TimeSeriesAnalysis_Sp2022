...
BR_Rain_ts <- ts(BR_complete$Rainfall,start=c(f_year,f_month),frequency = 12)
BR_Temp_ts <- ts(BR_complete$Temperature_C,start=c(f_year,f_month),frequency = 12)
>>>>>>> 04e0ccae56a145bdff9489edfda8bb12e6c218de
...
```{r}
MEAN_deseas <- meanf(BR_Temp_ts_deseas, h=36)
plot(MEAN_deseas)

NAIVE_deseas <- naive(BR_Temp_ts_deseas, h=36)
plot(NAIVE_deseas)

SMA_deseas_fit <- sma(y = BR_Temp_ts_deseas, h = 36, holdout = FALSE, silent = FALSE)
summary(SMA_deseas_fit)

SES_deseas_fit=ses( y = BR_Temp_ts_deseas, h = 36, holdout = FALSE, silent = FALSE)
...
```{r}
MEAN_seas <- meanf(y = BR_Temp_ts[,"Temperature"], h = 36)
plot(MEAN_seas)

SNAIVE_seas <- snaive(BR_Temp_ts[,1], h=36)
plot(SNAIVE_seas)

SMA_seas_fit <- sma( y = BR_Temp_ts[,1], h = 36, holdout = FALSE, silent = FALSE) #order=2
summary(SMA_seas_fit)

SES_seas_fit <- ses(y = BR_Temp_ts[,1], h = 36, holdout = FALSE, silent = FALSE)
summary(SES_seas_fit)
plot(SES_seas_fit)

SARIMA_autofit <- auto.arima(BR_Temp_ts[,1])
